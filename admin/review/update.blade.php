  ...
  <div class="formbold-main-wrapper">
    <div class="formbold-form-wrapper">
      <form action="{{route('admin.review.update', $review->id)}}" method="POST">
        ...
        <option value="{{$item->id}}" {{($item->id == $review->user_id) ? "selected" : ""}}>{{$item->name}}</option>
      @endforeach
          </select>
        </div>
        <div class="formbold-mb-5">
        ...
        <option value="{{$item->id}}" {{($item->id == $review->product_id) ? "selected" : ""}}>{{$item->name}}</option>
      @endforeach
          </select>
        </div>
        <div class="formbold-mb-5">
          <label for="rating" class="formbold-form-label"> Rating </label>
          <input type="number" name="rating" id="name" min="0" max="5" value="{{$review->rating}}" placeholder="Rating"
        ...
        <div class="formbold-mb-5">
          ...
          <textarea rows="6" name="comment" id="ckeditor" placeholder="Type your Comment"
            class="formbold-form-input">{{$review->comment}}</textarea>
        </div>
        <div>
